...
class FollowLineNode(Node):
    def __init__(self):
        ...
        self.max_thr = 0.15
        self.yaw_pid = PIDController(Kp=0.6, Ki=0.0, Kd=0.1, setpoint=0.0, output_limits=(-max_yaw, max_yaw))
        ...
        if self.started and not self.paused:
            ...
            if self.state == RobotState.FOLLOW_LINE:
                if light_color == 'red':
                    ...
                    self.last_state_change_time = now
                    self.immediate_command = (0.0, 0.0)
                ...
                if light_color == 'green' and self.expecting_straight_after_green:
                    ...
                    self.last_state_change_time = now
                    self.expecting_straight_after_green = False
                    self.immediate_command = (0.15, 0.095)
    ...
    def follow_line(self, frame):
        ...
        if contours:
            ...
            yaw = self.yaw_pid.compute(normalized_x)
            alignment = 1 - abs(normalized_x)
            align_thres = 0.3
            throttle = self.max_thr * ((alignment - align_thres) / (1 - align_thres)) if alignment >= align_thres else 0

            pt1, pt2, _, line_cx, line_cy = self.get_contour_line(best_contour)
    ...
    def show_robot_status(self, frame, light_color, detected_sign):
        status_frame = np.zeros((300, 500, 3), dtype=np.uint8)
        state_text = self.state.name if self.state != RobotState.EXECUTE_SIGN else f"EXECUTE_{self.last_sign_detected.upper()}"
