...
class FollowLineNode(Node):
    def __init__(self):
        ...
        self.get_logger().info('🟢 PID + YOLOv8 + Traffic light circle-based detection (separate window)')
    ...
    def detect_traffic_light_circles(self, frame):
        ...
        def detect_circles(mask, color_name, bgr_color):
            ...
            if circles is not None:
                ...
                for circle in np.uint16(np.around(circles[0])):
                    x, y, r = circle
                    cv2.circle(circle_frame, (x, y), r, bgr_color, 2)
                    cv2.circle(circle_frame, (x, y), 2, bgr_color, 3)
        ...
        cv2.imshow("Traffic Light Circles", cv2.resize(circle_frame, (640, 480)))
